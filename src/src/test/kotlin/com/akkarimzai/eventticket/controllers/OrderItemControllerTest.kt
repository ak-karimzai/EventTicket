...
class OrderItemControllerTest(
    ...
    fun `create and load order item for existing order`() {
        ...
        val ticketDtoList = webTestClient.get()
            .uri("/api/v1/categories/$categoryId/events/$eventId/tickets?page=0&size=10")
            .exchange()
            .expectStatus().isOk
    ...
    @Test
    fun `update existing order item by id`() {
        val category = CreateCategoryCommand("test category")
        val event = CreateEventCommand(
            title = "test event",
            artist = "test artist",
        ...
        val orderItemId = webTestClient.post()
            .uri("/api/v1/orders/$orderId/items")
            .header("Authorization", "Bearer $adminToken")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(newOrderItem)
        ...
        val updateOrderItemCommand = UpdateOrderItemCommand(
            amount = ThreadLocalRandom.current().nextInt(1, 20 + 1)
        )
        webTestClient.put()
            ...
            .header("Authorization", "Bearer $adminToken")
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(updateOrderItemCommand)
            .exchange()
